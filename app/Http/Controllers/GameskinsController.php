...
class GameskinsController extends GlobalController
{
    /**
    ...
    {
        return view('pages/gameskins')->with("data", $this->getViewData($sortType));
    }

    public function fetchGameskinsFromDatabase(Request $request) {
    ...
    private function getViewData($sortType) {
        ...
        $viewData = [
            'viewData' =>  [
                'gameskins' => $this->fetchGameskinsFromDatabase($defaultGameskinsRequest),
                'sortType' => $sortType,
                'page' => 'gameskins',
            ],
