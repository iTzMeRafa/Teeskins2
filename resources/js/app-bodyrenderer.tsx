...
import BodySkinRenderer from './components/BodySkinRenderer';
...
export default class AppBodySkinRenderer extends React.Component<{}, IAppUploadStates> {
        ...
        <>
          <div className="input-group mb-4">
            <div className="custom-file">
              <input type="file" className={inputClassName} id="bodyrenderer" required={true} onChange={() => this.handleFileInput(event)}/>
    ...
    return (
        <>
          ...
          <div className="form-group row mb-5">
            <label className="col-sm-4 col-form-label">Body Size*</label>
            <div className="col-sm-8">
              <select
  ...
  private handleAssetSizeChange() {
    ...
    const bodyElement = document.getElementById('1_bodyRender_assetPage') as HTMLImageElement;
    if (selectedAssetSize === "small" || selectedAssetSize === "default" ||selectedAssetSize === "large") {

      if (bodyElement) {
        bodyElement.classList.remove(`bodyRenderer--${this.state.assetSize}`);
        bodyElement.classList.add(`bodyRenderer--${selectedAssetSize}`);
      }
  ...
  private handleDownloadButtonClick (event: Event) {
    ...
    const bodyRenderSrc = (document.getElementById("1_bodyRender_assetPage") as HTMLImageElement).src;
    const saveAsName = (document.getElementById("saveAsName") as HTMLInputElement).value;
    const link = document.createElement('a');
    link.href = bodyRenderSrc;
  ...
  private handleFileInput (event) {
    ...
    const fileInputEl = document.getElementById('bodyrenderer') as HTMLInputElement;
  ...
  private renderPreview () {
    ...
    return (
        <BodySkinRenderer imagePath={this.state.tempFile} id="1" size={this.state.assetSize} locationType="assetPage"/>
    );
  }
}

if (document.getElementById('app')) {
  ReactDOM.render(<AppBodySkinRenderer />, document.getElementById('app'));
}
